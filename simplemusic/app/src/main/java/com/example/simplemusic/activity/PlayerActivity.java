...
public class PlayerActivity extends AppCompatActivity implements View.OnClickListener{
            ...
            if(item == null) {
                ...
                if (item.isOnlineMusic){
                    Glide.with(getApplicationContext())
                            .load(item.imgUrl)
                            .placeholder(R.drawable.defult_music_img)//显示默认图片
                            .error(R.drawable.defult_music_img)
                            .into(musicImgView);
                }
                else {
                    ContentResolver resolver = getContentResolver();
                    Bitmap img = Utils.getLocalMusicBmp(resolver, item.imgUrl);//获取专辑图片
                    Glide.with(getApplicationContext())
                            .load(img)
    ...
    @Override
    public boolean onOptionsItemSelected(MenuItem item) {//退出
        switch (item.getItemId()){
            case android.R.id.home:
                finish();
                return true;
        }
        return super.onOptionsItemSelected(item);//保存歌曲
    }
